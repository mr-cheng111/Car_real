...
CLaserOdometry2D::CLaserOdometry2D() :
  ...
  motion_filter_linear_m(0.0),
  motion_filter_angular_rad(0.0),
...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::msg::LaserScan& scan)
{
  ...
  const auto m_runtime =
    std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start).count();
  RCLCPP_DEBUG_THROTTLE(
    get_logger(), *get_clock(), 2000,
    "execution time (ms): %.3f", m_runtime);
...
void CLaserOdometry2D::PoseUpdate()
{
  ...
  double inc_yaw = double(kai_loc_(2) / fps);
  double inc_lin = std::hypot(double(acu_trans(0,2)), double(acu_trans(1,2)));
  if (inc_lin < motion_filter_linear_m && std::abs(inc_yaw) < motion_filter_angular_rad)
  {
    inc_yaw = 0.0;
    acu_trans(0,2) = 0.0f;
    acu_trans(1,2) = 0.0f;
  ...
  pose_aux_2D = rf2o::matrixYaw(inc_yaw);
  ...
  const bool has_motion =
    std::abs(double(acu_trans(0,2))) > 1e-9 ||
    std::abs(double(acu_trans(1,2))) > 1e-9 ||
    std::abs(inc_yaw) > 1e-9;
